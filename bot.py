...
def check_filters(direction: str, indicators: IndicatorSnapshot, code: str, funding_rate: Optional[float]=None, oi: Optional[float]=None, entry_price: Optional[float]=None) -> tuple[bool,str]:
    ...
    max_live_ema_distance = STRONG_TREND_MAX_ENTRY_DISTANCE_ATR if strong_trend_exception else MAX_ENTRY_DISTANCE_ATR
    ...
    for d in (['long','short'] if not prefer_direction else [prefer_direction]):
        if m.regime == 'range':
            ...
            if (
                ...
                candidates.append((min(sc,100),d,['برگشت از حمایت','RSI پایین محدوده','شیب RSI مثبت','MACD صعودی','حجم کافی'],[]))
            elif (
                d=='short'
                ...
                sc = 70 + (6 if m.volume_ratio >= VOLUME_STRONG else 0)
                candidates.append((min(sc,100),d,['برگشت از مقاومت','RSI بالای محدوده','شیب RSI منفی','MACD نزولی','حجم کافی'],[]))
        else:
            ...
            # fallback مستقل برای روندهای واقعاً قوی: به‌جای پایین‌آوردن کورکورانه‌ی score،
            # قدرت روند (ADX/DI) و RSI جهت‌دار را مستقیماً می‌سنجد. فیلترهای نهایی
            # check_filters همچنان اجباری هستند؛ range اصلاً وارد این مسیر نمی‌شود.
            strong_fallback = (
            ...
            # در trend معمولی، وجود تأیید روند 1H هم می‌تواند جای BOS/Retest را بگیرد؛
            # اما فقط وقتی score به حداقل لازم رسیده باشد. این مسیر برای range هرگز فعال نیست.
            ...
            if standard_gate or fallback_gate:
                ...
                candidates.append((effective_score,d,rs,wr))
    if not candidates:
        diag_parts = []
