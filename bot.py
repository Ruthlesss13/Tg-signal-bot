...
async def evaluate_coin(code: str, account_balance_usdt: float=1000.0, skip_cooldown: bool=False, prefer_direction: Optional[str]=None, cooldown_seconds: Optional[float]=None):
    ...
    if h is not None and h.regime == "range" and h.adx < 24:
        ...
        if setup_type not in ("range_reversal", "range_scalp"):
            return None, f"رژیم رنج ضعیف ۴H (ADX4h={h.adx:.1f}) — فقط برگشت رنج / نوسان‌گیر مجاز است", None
    ...
    # فیلترهای سخت روندی برای نوسان‌گیر اعمال نمی‌شود (منطق خودش را دارد)
    if setup_type != "range_scalp":
