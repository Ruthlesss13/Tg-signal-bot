...
# سبک نوسان‌گیر (مخصوص بازار رنج) — تارگت کوچک، استاپ حدود ۲٪
RANGE_SCALP_TP_PCT = 0.013          # هدف ≈ ۱.۳٪
RANGE_SCALP_SL_PCT = 0.020          # حد ضرر ≈ ۲٪
...
async def evaluate_coin(code: str, account_balance_usdt: float=1000.0, skip_cooldown: bool=False, prefer_direction: Optional[str]=None, cooldown_seconds: Optional[float]=None):
    ...
    for d in (['long','short'] if not prefer_direction else [prefer_direction]):
        ...
        # ---- 3) vol_momentum: ستاپ جدید برای نوسان جهت‌دار (سخت‌گیرانه‌تر برای کیفیت) ----
        # مناسب بازارهای پرنوسان: حجم قوی + کندل جهت‌دار + مومنتوم RSI/MACD + DI
        # برای عبور از check_filters حتماً trend یا BOS/Retest لازم است.
    ...
    # اگر تاریخچه مشابه ضعیف باشد، سیگنال را رد کن (سخت‌گیرانه‌تر برای کیفیت)
    if hist_est.get("pct") is not None and hist_est.get("sample", 0) >= 20 and hist_est["pct"] < 48:
