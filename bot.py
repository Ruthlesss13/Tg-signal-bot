...
def update_signal_status(signal_id: int, status: str):
    with _conn() as c:
        if status == "open":
            c.execute("UPDATE signals SET status='open', closed_ts=NULL WHERE id=? AND status='open'", (signal_id,))
        else:
            c.execute("UPDATE signals SET status=?, closed_ts=? WHERE id=? AND status='open'", (status, time.time(), signal_id))
        return c.rowcount > 0
...
def _drop_incomplete_candle(df: pd.DataFrame, timeframe: str) -> pd.DataFrame:
    """فقط کندل‌های بسته‌شده را برای اندیکاتورها نگه می‌دارد.

    OHLCV صرافی معمولاً آخرین کندل در حال تشکیل را هم برمی‌گرداند.
    استفاده از آن باعث می‌شود RSI/ADX/Volume/ATR و حتی قیمت ورود در طول کندل
    تغییر کنند و سیگنال با داده‌ای صادر شود که هنوز نهایی نشده است.
    """
    if df is None or df.empty or "timestamp" not in df.columns:
        return df
    minutes = {"1m": 1, "5m": 5, "15m": 15, "30m": 30, "1h": 60, "4h": 240, "1d": 1440}.get(timeframe)
    if not minutes:
        return df
    out = df.copy()
    ...
    # شروع کندل جاری بر اساس epoch؛ کندل با start == current_start هنوز بسته نشده.
    current_start = (now_ms // tf_ms) * tf_ms
    out = out[(ts.astype("int64") // 1_000_000) < current_start].copy()
    return out.reset_index(drop=True)

def compute_indicators(df: pd.DataFrame, timeframe: str = MAIN_TF) -> Optional[IndicatorSnapshot]:
    if df is None:
        return None
    df = _drop_incomplete_candle(df, timeframe)
    if len(df) < max(EMA_SLOW,VOLUME_MA_PERIOD,STRUCTURE_LOOKBACK)+20: return None
...
def check_filters(direction: str, indicators: IndicatorSnapshot, code: str, funding_rate: Optional[float]=None, oi: Optional[float]=None) -> tuple[bool,str]:
    ...
    if fr is not None:
        if direction == "long" and fr > FUNDING_RATE_MAX_FOR_LONG:
            return False, f"فاندینگ بیش‌ازحد مثبت است ({fr:.4%}) — لانگ پرهزینه"
        if direction == "short" and fr < -FUNDING_RATE_MAX_FOR_LONG:
            return False, f"فاندینگ بیش‌ازحد منفی است ({fr:.4%}) — شورت پرهزینه"
...
def build_risk_plan(direction: str, entry: float, atr: float, account_balance_usdt: float = 1000.0, structure_target: Optional[float] = None) -> RiskPlan:
    ...
    if direction not in ("long", "short"):
        return RiskPlan(direction, entry, 0.0, [], leverage, 0.0, 0.0, False, "جهت معامله نامعتبر")
    if not all(pd.notna(x) and float(x) > 0 for x in (entry, atr, account_balance_usdt)):
        return RiskPlan(direction, entry, 0.0, [], leverage, 0.0, 0.0, False, "قیمت/ATR/موجودی نامعتبر")
    stop_distance = float(atr) * ATR_STOP_MULTIPLIER
    if not pd.notna(stop_distance) or stop_distance <= 0 or stop_distance >= entry:
        return RiskPlan(direction, entry, 0.0, [], leverage, 0.0, 0.0, False, "فاصله حد ضرر نامعتبر")
    if direction == "long":
...
async def analyze_coin_full_status(code: str) -> str:
    ...
    try:
        ...
        # تمام محاسبات تکنیکال روی آخرین کندل بسته‌شده انجام می‌شوند.
        df_1h_closed = _drop_incomplete_candle(df_1h, "1h")
        df_4h_closed = _drop_incomplete_candle(df_4h, "4h") if df_4h is not None else None
        df_1d_closed = _drop_incomplete_candle(df_1d, "1d") if df_1d is not None else None
        if len(df_1h_closed) < max(EMA_SLOW, VOLUME_MA_PERIOD, STRUCTURE_LOOKBACK) + 20:
            return f"❌ داده‌های بسته‌شده کافی برای تحلیل **{code}** در دسترس نیست."

        ind = compute_indicators(df_1h_closed, "1h")
        ind_4h = compute_indicators(df_4h_closed, "4h") if df_4h_closed is not None else None
        ind_1d = compute_indicators(df_1d_closed, "1d") if df_1d_closed is not None else None
        ...
        close_series = df_1h_closed["close"]
        ...
        low_24h = price
        volume_24h = 0
        if df_1d_closed is not None and len(df_1d_closed) > 1:
            close_24h = df_1d_closed["close"].iloc[-2] if len(df_1d_closed) > 1 else price
            change_24h = ((price - close_24h) / close_24h * 100) if close_24h else 0
            high_24h = df_1d_closed["high"].iloc[-1]
            low_24h = df_1d_closed["low"].iloc[-1]
            volume_24h = df_1d_closed["volume"].iloc[-1]
        ...
        if df_1d_closed is not None and len(df_1d_closed) >= 7:
            support_weekly = df_1d_closed["low"].tail(7).min()
            resistance_weekly = df_1d_closed["high"].tail(7).max()
        else:
        ...
        ema20 = EMAIndicator(df_1h_closed["close"], window=20).ema_indicator().iloc[-1]
        ema50 = EMAIndicator(df_1h_closed["close"], window=50).ema_indicator().iloc[-1]
        ema200 = EMAIndicator(df_1h_closed["close"], window=200).ema_indicator().iloc[-1]

        macd_ind = MACD(df_1h_closed["close"], window_fast=MACD_FAST, window_slow=MACD_SLOW, window_sign=MACD_SIGNAL)
        macd_line = macd_ind.macd().iloc[-1]
        macd_signal = macd_ind.macd_signal().iloc[-1]
        macd_hist = macd_ind.macd_diff().iloc[-1]

        bb_ind = BollingerBands(df_1h_closed["close"], window=20)
        bb_upper = bb_ind.bollinger_hband().iloc[-1]
        bb_middle = bb_ind.bollinger_mavg().iloc[-1]
        bb_lower = bb_ind.bollinger_lband().iloc[-1]

        stoch_rsi = StochRSIIndicator(df_1h_closed["close"], window=14).stochrsi().iloc[-1] * 100 if len(df_1h_closed) > 20 else 50
...
# ===========================
def get_signal_strength_and_confidence(ind: IndicatorSnapshot, funding: Optional[float], oi: Optional[float], signal_score: Optional[float] = None, direction: Optional[str] = None) -> tuple[str,str,int]:
    """قدرت پیام باید دقیقاً از همان scoreی بیاید که ستاپ را تأیید کرده است.

    هرگز score را از روی trend_up حدس نمی‌زنیم؛ چون ممکن است سیگنال لانگ با
    ساختار/پولبک صادر شده باشد در حالی که trend_up لحظه‌ای False است.
    """
    if signal_score is None:
        d = direction or ("long" if ind.trend_up else "short")
        signal_score, _, _ = setup_score(d, ind, ind, funding)
    score = max(0, min(100, int(round(float(signal_score)))))
...
def format_signal_message(code: str, direction: str, plan: RiskPlan, setup_type: str, ai_status: str, ai_text: str, ind: IndicatorSnapshot, funding: Optional[float], oi: Optional[float], rate: float, hist_est: Optional[dict] = None, signal_score: Optional[float] = None) -> str:
    ...
    strength, confidence_text, score = get_signal_strength_and_confidence(ind, funding, oi, signal_score=signal_score, direction=direction)
...
def _parse_ai_decision(raw_s: str, expected: str) -> tuple[str, str]:
    ...
    # بدون DECISION صریح، هیچ حدسی از روی کلمات BUY/SELL انجام نمی‌شود.
    # این کار جلوی تأیید اشتباه به‌علت مثال‌ها، reasoning یا متن قالبی مدل را می‌گیرد.
    return "rejected", f"{reason} | ارزیابی AI: {conf}%"
...
def confirm_signal_with_ai(chart_png: Optional[bytes], context: dict) -> tuple[str, str]:
    ...
    try:
        ...
        if not raw_s:
            # پاسخ خالی هرگز نباید با یک prompt مصنوعی به تأیید تبدیل شود.
            # در این حالت فقط وضعیت AI نامشخص است و موتور تصمیم‌گیری باید آن را
            # به‌عنوان «عدم دسترسی» نگه دارد، نه تأیید.
            logger.warning(f"AI empty content; no synthetic confirmation. keys={list(((data.get('choices') or [{}])[0].get('message') or {}).keys())}")
            return "unavailable", "پاسخ خالی از مدل"
...
async def evaluate_coin(code: str, account_balance_usdt: float=1000.0, skip_cooldown: bool=False, prefer_direction: Optional[str]=None, cooldown_seconds: Optional[float]=None):
    ...
    m,h=compute_indicators(dm, MAIN_TF),compute_indicators(dh, HIGHER_TF)
    if m is None or h is None: return None,'اندیکاتورها محاسبه نشدند',None
    # سیگنال و مانیتور هر دو بر پایه قرارداد دائمی همان صرافی هستند.
    # اختلاف شدید قیمت لحظه‌ای با آخرین کلوز بسته‌شده یعنی ورود دقیقاً روی آن کلوز
    # دیگر قابل تضمین نیست؛ سیگنال را رد می‌کنیم تا entry جعلی ساخته نشود.
    live_price = _to_float(fd.get("last_price"), None)
    if live_price is None or live_price <= 0:
        return None,'قیمت لحظه‌ای قرارداد دائمی معتبر نیست',None
    entry_gap = abs(live_price - m.close) / m.close if m.close else 999
    if entry_gap > max(0.01, 0.75 * (m.atr / m.close if m.close else 999)):
        return None,f'فاصله قیمت لحظه‌ای تا آخرین کلوز بسته‌شده زیاد است ({entry_gap*100:.2f}%)',None
    candidates=[]
...
async def _publish_signal(app: Application, signal_id: int, payload: tuple):
    ...
    signal_score = payload[10] if len(payload) > 10 else None
    code = None
    with _conn() as c:
    ...
    if not code:
        return
    text = format_signal_message(code, direction, plan, setup_type, ai_status, ai_text, ind, funding, oi, rate, hist_est=hist_est, signal_score=signal_score)
    sent_channel = None
...
async def _finalize_signal(app: Application, row, result: str, price: float, lesson: str):
    ...
    if not update_signal_status(row["id"], status):
        logger.warning(f"Signal {row['id']} already finalized; skipping duplicate finalize")
        return
