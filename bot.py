...
GROQ_MODEL = os.getenv("GROQ_MODEL", "llama-3.1-8b-instant")  # پایدار روی کلید رایگان Groq
...
def _ai_compact_context(context: dict) -> dict:
    ...
    return {
        ...
        "stop": risk.get("stop"),
        "rr": risk.get("rr"),
        "reasons": (context.get("rule_reasons") or [])[:6],
        "warnings": (context.get("rule_warnings") or [])[:4],
    }
...
def confirm_signal_with_ai(chart_png: Optional[bytes], context: dict) -> tuple[str, str]:
    ...
    prompt = (
        "Crypto futures setup review. Reply with ONLY one JSON object, no markdown.\n"
        'Keys: "decision" (BUY|SELL|NO_TRADE), "confidence" (0-100), "reason_fa" (short Persian).\n'
        "Reject weak/stretched entries. ADX is strength not direction.\n"
    ...
    try:
        try:
            data = _post(use_json_mode=True)
        except urllib.error.HTTPError as e:
            # بعضی مدل‌ها json_mode را پشتیبانی نمی‌کنند → بدون آن دوباره تلاش
            err_body = ""
            try:
                ...
                pass
            if e.code == 400:
                logger.warning(f"AI HTTP 400 (retry without json_mode): {err_body}")
                data = _post(use_json_mode=False)
            else:
        ...
        try:
            ...
            if start >= 0 and end > start:
                ...
                if expected in up and "NO_TRADE" not in up and "REJECT" not in up:
                    return "confirmed", raw_s[:400]
                return "rejected", raw_s[:400]
