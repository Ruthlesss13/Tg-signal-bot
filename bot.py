...
def _parse_ai_decision(raw_s: str, expected: str) -> tuple[str, str]:
    ...
    if expected in up or (expected == "BUY" and "LONG" in up) or (expected == "SELL" and "SHORT" in up):
...
def confirm_signal_with_ai(chart_png: Optional[bytes], context: dict) -> tuple[str, str]:
    ...
    prompt = (
        f"Setup for {compact.get('ticker', '?')} direction={expected}. "
        f"score={compact.get('score')} rsi={compact.get('rsi')} adx={compact.get('adx')} "
        f"vol={compact.get('vol')} rr={compact.get('rr')}. "
        f"Reply ONE line only: DECISION={expected} or NO_TRADE | CONF=0-100 | REASON=short Persian"
    )
    ...
    try:
        data = _call(
            [
                {"role": "system", "content": "Crypto futures filter. One short plain line answer only."},
