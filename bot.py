...
def estimate_success_from_history(direction: str, setup_type: str, rsi: float, adx: float, vol: float, had_bos: bool) -> dict:
    ...
    with _conn() as c:
        rows = c.execute(
            ...
            "WHERE direction=? AND result IN ('tp3', 'sl') ORDER BY created_ts DESC LIMIT 80",
            (direction,)
        ).fetchall()
    if not rows:
...
def check_filters(direction: str, indicators: IndicatorSnapshot, code: str, funding_rate: Optional[float]=None, oi: Optional[float]=None) -> tuple[bool,str]:
    is_range_reversal = indicators.adx < ADX_TREND_MIN and (indicators.edge_reversal_up or indicators.edge_reversal_down)
    if indicators.adx < ADX_TREND_MIN and not is_range_reversal and not (indicators.bos_up or indicators.bos_down):
    ...
    if is_range_reversal:
        if direction == "long":
            if not indicators.edge_reversal_up or not (RSI_OVERSOLD <= indicators.rsi <= 42):
                return False, "شرایط برگشت از کف برای لانگ کامل نیست"
            if indicators.macd_cross_down:
                return False, "MACD تقاطع نزولی — خلاف برگشت لانگ"
        else:
            if not indicators.edge_reversal_down or not (58 <= indicators.rsi <= RSI_OVERBOUGHT):
                return False, "شرایط برگشت از سقف برای شورت کامل نیست"
    ...
    def _call(messages, max_tokens=220):
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "Mozilla/5.0 (compatible; SignalBot/4.1.5)",
            },
            method="POST",
        )
                ...
                for row in rows:
                    try:
                        ...
                        if new_highest > highest:
                            ...
                            # تجربه فقط در نتیجه نهایی ثبت می‌شود؛ TP1/TP2 معامله را نبسته‌اند.
                            if new_highest >= 3:
                                try:
                                    save_experience(row["id"], code, direction, row["setup_type"] or "", "tp3", features, duration, lesson)
                                except Exception as e:
                                    logger.warning(f"save_experience TP3: {e}")
