...
def format_signal_message(code: str, direction: str, plan: RiskPlan, setup_type: str, ai_status: str, ai_text: str, ind: IndicatorSnapshot, funding: Optional[float], oi: Optional[float], rate: float) -> str:
    ...
    text = (
        ...
        f"{rtl}🎯 **ضریب اطمینان:** {confidence_text}\n\n"
        f"{rtl}💰 **قیمت ورود:**\n"
        ...
        f"{rtl}   نسبت ریسک به ریوارد: `1:{plan.risk_reward:.2f}`\n\n"
        f"{rtl}📊 **داده‌های تکمیلی:**\n"
        f"{rtl}   RSI: `{ind.rsi:.1f}`\n"
        f"{rtl}   ADX: `{ind.adx:.1f}`\n"
        f"{rtl}   حجم: `{ind.volume_ratio:.2f}x`\n"
        f"{rtl}   فاندینگ‌ریت: `{funding_text}`\n"
            ...
            for row in open_signals():
                ...
                if hit_sl:
                    text = (
                        ...
                        f"{rtl}ورود: `{row['entry']:.6f}` | SL: `{stop_loss:.6f}`\n"
                        f"{rtl}`{shamsi_now()}`"
                    )
                ...
                if highest < 2:
                    if (price >= targets[1] if direction == "long" else price <= targets[1]):
                        text = (
                            f"{rtl}💰 **TP2 زده شد** — #{code}\n"
                            f"{rtl}حد ضرر به TP1 منتقل شد: `{targets[0]:.6f}`\n"
                            f"{rtl}`{shamsi_now()}`"
                        )
                        if CHANNEL_ID and reply_id:
                            await app.bot.send_message(CHANNEL_ID, text, parse_mode="Markdown", reply_to_message_id=reply_id)
                ...
                if signal_id not in _trend_warned and CHANNEL_ID and reply_id:
                    try:
                        ...
                        if flipped:
                            if highest == 0:
                                stage = "قبل از رسیدن به هیچ تارگتی"
                            else:
                                stage = f"پس از TP{highest}"
                            text = (
                                f"{rtl}⚠️ **هشدار تغییر روند** — #{code}\n"
                                f"{rtl}سیگنال باز ({'لانگ' if direction == 'long' else 'شورت'}) — {stage}\n"
                                f"{rtl}دلیل: {' | '.join(reason_bits)}\n"
                                f"{rtl}لطفاً مدیریت ریسک را بررسی کنید.\n"
                                f"{rtl}`{shamsi_now()}`"
                            )
                            await app.bot.send_message(CHANNEL_ID, text, parse_mode="Markdown", reply_to_message_id=reply_id)
                            ...
                            if highest == 0 and ind_4h and (
                                ...
                                inv_text = (
                                    f"{rtl}🚫 **سیگنال باطل شد (تغییر روند قوی)** — #{code}\n"
