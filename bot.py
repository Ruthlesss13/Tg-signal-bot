...
def record_signal(ticker, direction, entry, stop_loss, targets, leverage, risk_reward, ai_confirmed, ai_raw_text, setup_type: str = "", features: Optional[dict] = None) -> int:
    with _conn() as c:
        cur = c.execute(
            "INSERT INTO signals (ticker, direction, entry, stop_loss, targets, leverage, risk_reward, ai_confirmed, ai_raw_text, created_ts, setup_type, features_json) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
...
def save_experience(signal_id: int, ticker: str, direction: str, setup_type: str, result: str,
    ...
    with _conn() as c:
        c.execute(
            ...
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                signal_id, ticker, direction, setup_type or "", result,
                _to_float(features.get("rsi")), _to_float(features.get("adx")),
                _to_float(features.get("vol")), _to_float(features.get("score")),
                1 if features.get("had_bos") else 0,
                duration_sec, lesson, time.time(),
            ),
        )
...
def estimate_success_from_history(direction: str, setup_type: str, rsi: float, adx: float, vol: float, had_bos: bool) -> dict:
    ...
    wins = sum(1 for r in pool if str(r["result"]).startswith("tp"))
    losses = sum(1 for r in pool if r["result"] == "sl")
    closed = wins + losses
    if closed < 3:
...
def setup_score(direction: str, m: IndicatorSnapshot, h: IndicatorSnapshot, funding: Optional[float], code: Optional[str] = None) -> tuple[int,list[str],list[str]]:
    ...
    checks=[
        (m.trend_up if direction=='long' else m.trend_down,18,'روند 1H هم‌جهت','trend_1h'),
        (h.trend_up if direction=='long' else h.trend_down,14,'روند 4H هم‌جهت','trend_4h'),
        ((m.plus_di>m.minus_di+MIN_DI_GAP) if direction=='long' else (m.minus_di>m.plus_di+MIN_DI_GAP),10,'DI هم‌جهت','di'),
        (m.adx>=ADX_TREND_MIN,8,'قدرت روند','adx'),
        ((m.bos_up or m.retest_long) if direction=='long' else (m.bos_down or m.retest_short),18,'ساختار/پولبک تأییدشده','structure'),
        (rsi_ok and ((m.rsi_slope>0) if direction=='long' else (m.rsi_slope<0)),10,'مومنتوم RSI در بازه','rsi'),
        (macd_ok,8,'MACD هم‌جهت بدون تقاطع مخالف','macd'),
        (m.volume_ratio>=VOLUME_MIN_RATIO,5,'حجم کافی','volume_min'),
        (m.volume_ratio>=VOLUME_STRONG,4,'حجم قوی','volume_strong'),
        (m.distance_to_ema_atr<=MAX_ENTRY_DISTANCE_ATR,5,'ورود نزدیک به EMA','ema_distance'),
    ]
    score_parts = {}
    for ok,pts,label,key in checks:
        awarded = pts if ok else 0
        score_parts[key] = awarded
        if ok:
            score+=pts; reasons.append(label)
    fr = _to_float(funding, None)
    funding_points = 2 if fr is not None and ((direction == "short" and fr > 0) or (direction == "long" and fr < 0)) else 0
    score_parts['funding'] = funding_points
    if funding_points:
    ...
    # دیباگ نسخه 4.1.4: اجزای امتیاز برای هر ارز/جهت جداگانه ثبت می‌شوند تا علت رد دقیقاً مشخص باشد.
    if code:
        logger.info(
            f"🧮 SCORE {code} {direction.upper()}: total={final_score}/100 | "
            f"1H={score_parts['trend_1h']} 4H={score_parts['trend_4h']} DI={score_parts['di']} "
            f"ADX={score_parts['adx']} STRUCT={score_parts['structure']} RSI={score_parts['rsi']} "
            f"MACD={score_parts['macd']} VOL={score_parts['volume_min']}+{score_parts['volume_strong']} "
            f"EMA={score_parts['ema_distance']} FUND={score_parts['funding']} | "
            f"ADXv={m.adx:.1f} RSIv={m.rsi:.1f} VOLv={m.volume_ratio:.2f} "
            f"DIv={m.plus_di:.1f}/{m.minus_di:.1f} BOS={m.bos_up}/{m.bos_down} "
            f"Retest={m.retest_long}/{m.retest_short} EMA_ATR={m.distance_to_ema_atr:.2f}"
        )
...
def confirm_signal_with_ai(chart_png: Optional[bytes], context: dict) -> tuple[str, str]:
    ...
    try:
        data = _call(
            [
                {"role": "system", "content": "You filter crypto futures setups. Reply one line only. REASON must be short Persian. No English sentences."},
...
async def evaluate_coin(code: str, account_balance_usdt: float=1000.0, skip_cooldown: bool=False, prefer_direction: Optional[str]=None, cooldown_seconds: Optional[float]=None):
    ...
    for d in (['long','short'] if not prefer_direction else [prefer_direction]):
        if m.regime == 'range':
            ...
            sc,rs,wr=setup_score(d,m,h,funding,code)
    ...
    if not candidates:
        # علت رد را دقیق و قابل دیباگ ثبت کن؛ متن قبلی دو علت متفاوت را با «یا» مخلوط می‌کرد.
        if m.regime == 'range':
            reject_reason = 'reversal setup conditions not met'
        else:
            direction_reasons = []
            for d in (['long','short'] if not prefer_direction else [prefer_direction]):
                sc, _, _ = setup_score(d, m, h, funding, code)
                structural = (m.bos_up or m.retest_long) if d == 'long' else (m.bos_down or m.retest_short)
                strong_trend = (
                    (m.trend_up if d=='long' else m.trend_down)
                    and (m.plus_di > m.minus_di + MIN_DI_GAP if d=='long' else m.minus_di > m.plus_di + MIN_DI_GAP)
                ...
                if sc < MIN_SIGNAL_SCORE:
                    direction_reasons.append(f'{d}:score={sc}<{MIN_SIGNAL_SCORE}')
                if not (structural or strong_trend):
                    direction_reasons.append(f'{d}:structure/strong_trend=False')
            reject_reason = ' | '.join(direction_reasons) if direction_reasons else 'no valid direction'
        return None,f'ستاپ رد شد | regime={m.regime} | {reject_reason} | ADX={m.adx:.1f} RSI={m.rsi:.1f} +DI/-DI={m.plus_di:.1f}/{m.minus_di:.1f} BOS={m.bos_up}/{m.bos_down}',None
    ...
    sid = record_signal(
        code, direction, plan.entry, plan.stop_loss, plan.targets, plan.leverage, plan.risk_reward,
        ai_status == 'confirmed', ai_text, setup_type, features=features,
    )
    rate = fetch_irt_rate_sync()
    return sid, 'سیگنال صادر شد', (plan, setup_type, ai_status, ai_text, direction, m, funding, oi, rate, hist_est)
            ...
            if open_positions_count() >= MAX_CONCURRENT_POSITIONS:
                ...
                for code, result in results:
                    ...
                    if signal_id and payload:
                        if published >= slots:
                            # بیش از سقف همزمان — سیگنال ثبت‌شده را باطل کن تا DB شلوغ نشود
                            try:
                                update_signal_status(signal_id, "invalidated")
                            except Exception:
                                pass
                            logger.info(f"⏸ {code}: سقف پوزیشن همزمان پر شد — سیگنال منتشر نشد")
                            continue
                ...
                logger.info(f"🔎 Scan complete: {len(COIN_CODES)} coins | published={published}/{slots} | {time.time()-started:.1f}s")
                ...
                for row in rows:
                    try:
                        ...
                        if hit_sl:
                            ...
                            lesson = build_postmortem(direction, "sl", features, duration, price, entry)
                            try:
                                save_experience(row["id"], code, direction, row["setup_type"] or "", "sl", features, duration, lesson)
                            except Exception as e:
                                logger.warning(f"save_experience SL: {e}")
                        ...
                        if new_highest > highest:
                            ...
                            try:
                                ...
                                features = {}
                            duration = max(0.0, time.time() - float(row["created_ts"] or time.time()))
                            lesson = build_postmortem(direction, result_tag, features, duration, price, entry)
                            try:
                                save_experience(row["id"], code, direction, row["setup_type"] or "", result_tag, features, duration, lesson)
                            except Exception as e:
                                logger.warning(f"save_experience TP: {e}")
