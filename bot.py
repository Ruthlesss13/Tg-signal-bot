            ...
            async with scan_lock:
                ...
                if slots <= 0:
                    ...
                    for score, code, payload in selected:
                        ...
                        # --- چک تازگی قیمت درست قبل از انتشار (جلوگیری از سیگنال با قیمت عقب‌افتاده) ---
                        try:
                            ...
                            if drift > MAX_PRICE_DRIFT_PCT:
                                ...
                                continue
                            # اگر اختلاف کوچک بود، قیمت ورود را به تازه به‌روز کن و SL/TP را متناسب نگه ندار
                            # (برای سادگی و حفظ RR محاسبه‌شده، فقط در صورت اختلاف خیلی کوچک از قیمت تحلیل استفاده می‌کنیم)
                            # اینجا فقط لاگ می‌کنیم که قیمت تأیید شد.
                            logger.info(f"✅ {code}: قیمت تأیید شد (drift={drift*100:.3f}%)")
                        except Exception as pe:
                            logger.warning(f"⏭ {code}: خطای چک قیمت تازه: {pe} — سیگنال لغو شد")
                            continue
