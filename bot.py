...
def setup_score(direction: str, m: IndicatorSnapshot, h: IndicatorSnapshot, funding: Optional[float]) -> tuple[int,list[str],list[str]]:
    ...
    if funding is not None and ((direction=='short' and funding>0) or (direction=='long' and funding<0)): score+=2; reasons.append('فاندینگ به نفع جهت')
...
# ===========================
AI_PROMPT_TEMPLATE = """You are the independent final analyst for a crypto futures signal engine.
Do NOT blindly confirm. Decide BUY, SELL or NO_TRADE from the evidence. Penalize contradictions and poor entry location. ADX is strength, not direction. A rule score is not a win probability.
Return JSON only: {\"decision\":\"BUY|SELL|NO_TRADE\",\"confidence\":0-100,\"reason_fa\":\"...\",\"strength_fa\":\"ضعیف|متوسط|قوی|بسیار قوی\",\"warnings_fa\":[\"...\"],\"invalidation_fa\":\"...\"}
DATA:
{payload}"""

def confirm_signal_with_ai(chart_png: Optional[bytes], context: dict) -> tuple[str,str]:
    if not GROQ_API_KEY: return 'unavailable','کلید هوش مصنوعی تنظیم نشده است'
    prompt=AI_PROMPT_TEMPLATE.format(payload=json.dumps(context,ensure_ascii=False,default=str))
    body=json.dumps({'model':GROQ_MODEL,'messages':[{'role':'system','content':'You are a skeptical quantitative crypto analyst. Analyze independently; reject contradictory setups.'},{'role':'user','content':prompt}],'temperature':0.15,'max_tokens':700,'response_format':{'type':'json_object'}},ensure_ascii=False).encode()
    req=urllib.request.Request(f'{GROQ_BASE_URL}/chat/completions',data=body,headers={'Authorization':f'Bearer {GROQ_API_KEY}','Content-Type':'application/json'},method='POST')
    try:
        with urllib.request.urlopen(req,timeout=AI_TIMEOUT_SECONDS) as r: data=json.loads(r.read().decode())
        raw=(data.get('choices') or [{}])[0].get('message',{}).get('content',''); obj=json.loads(raw)
        decision=str(obj.get('decision','NO_TRADE')).upper(); conf=max(0,min(100,int(float(obj.get('confidence',0)))))
        reason=str(obj.get('reason_fa','')).strip(); warnings=obj.get('warnings_fa',[]) or []; inv=str(obj.get('invalidation_fa','')).strip()
        text=reason + ((' هشدار: '+'؛ '.join(map(str,warnings))) if warnings else '') + (f' ابطال: {inv}' if inv else '') + f' | ارزیابی مستقل AI: {conf}%'
        expected='BUY' if context.get('direction')=='long' else 'SELL'
        if decision==expected: return 'confirmed',text[:1200]
        return 'rejected',text[:1200] if decision!='NO_TRADE' else text[:1200]
    except Exception as e:
        logger.warning(f'AI analysis failed: {e}'); return 'unavailable',f'تحلیل AI در دسترس نبود: {e}'
...
async def evaluate_coin(code: str, account_balance_usdt: float=1000.0, skip_cooldown: bool=False, prefer_direction: Optional[str]=None, cooldown_seconds: Optional[float]=None):
    ...
    for d in (['long','short'] if not prefer_direction else [prefer_direction]):
        if m.regime == 'range':
            ...
            sc,rs,wr=setup_score(d,m,h,funding); structural=(m.bos_up or m.retest_long) if d=='long' else (m.bos_down or m.retest_short)
            if sc>=MIN_SIGNAL_SCORE and structural: candidates.append((sc,d,rs,wr))
    if not candidates: return None,f'ستاپ باکیفیت شکل نگرفت | regime={m.regime} ADX={m.adx:.1f} RSI={m.rsi:.1f} +DI/-DI={m.plus_di:.1f}/{m.minus_di:.1f} BOS={m.bos_up}/{m.bos_down}',None
            ...
            if open_positions_count() >= MAX_CONCURRENT_POSITIONS:
                ...
                for code, result in results:
                    ...
                    if signal_id and payload:
                        ...
                        published += 1
                    elif reason and not reason.startswith(("ستاپ باکیفیت", "در کول‌داون", "سیگنال باز", "داده OHLCV")):
