...
async def evaluate_coin(code: str, account_balance_usdt: float=1000.0, skip_cooldown: bool=False, prefer_direction: Optional[str]=None, cooldown_seconds: Optional[float]=None):
    ...
    for d in (['long','short'] if not prefer_direction else [prefer_direction]):
        if m.regime == 'range':
            ...
            strong_fallback = (
                strong_trend
                and di_gap >= STRONG_TREND_DI_GAP_MIN
            ...
            fallback_gate = strong_fallback and effective_score >= STRONG_TREND_FALLBACK_SCORE
    ...
    if not candidates:
        ...
        for diag_d in diag_dirs:
            ...
            strong_fallback = strong_trend and di_gap >= STRONG_TREND_DI_GAP_MIN and rsi_direction_ok
            effective_diag_score = min(100, dscore + STRONG_TREND_SCORE_BONUS) if strong_fallback else dscore
            if strong_fallback and effective_diag_score >= STRONG_TREND_FALLBACK_SCORE:
                if not structural:
                    gates.append(f'fallback روند قوی: effective_score={effective_diag_score}>={STRONG_TREND_FALLBACK_SCORE}')
