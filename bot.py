...
def confirm_signal_with_ai(chart_png: Optional[bytes], context: dict) -> tuple[str, str]:
    ...
    prompt = (
        f"Ticker {compact.get('ticker', '?')} want {expected}. "
        f"score={compact.get('score')} rsi={compact.get('rsi')} adx={compact.get('adx')} "
        f"vol={compact.get('vol')} rr={compact.get('rr')} "
        f"bos_up={compact.get('bos_up')} bos_down={compact.get('bos_down')}. "
        f"If setup is good reply exactly: DECISION={expected}|CONF=75|REASON=دلیل کوتاه فارسی "
        f"If weak reply exactly: DECISION=NO_TRADE|CONF=40|REASON=دلیل کوتاه فارسی "
        f"No other text."
    )
