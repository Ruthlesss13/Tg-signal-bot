...
async def evaluate_coin(code: str, account_balance_usdt: float=1000.0, skip_cooldown: bool=False, prefer_direction: Optional[str]=None, cooldown_seconds: Optional[float]=None):
    ...
    for d in (['long','short'] if not prefer_direction else [prefer_direction]):
        # Trend setup is evaluated independently of the regime. This prevents a
        # high-quality directional setup (e.g. score=78 with confirmed BOS) from
        # being discarded merely because ADX temporarily classifies the market as
        # range. Range-reversal remains a fallback only when no trend setup exists.
        ...
        if sc>=MIN_SIGNAL_SCORE and (structural or strong_trend):
            ...
            if m.regime == 'range':
                wr = list(wr) + ['ADX در محدوده Range است، اما ستاپ جهت‌دار امتیاز و ساختار کافی دارد']
            candidates.append((sc,d,rs,wr))

        # Range reversal is deliberately lower priority than a qualified trend
        # setup, and is used only as an alternative candidate.
        if m.regime == 'range':
            if d=='long' and m.edge_reversal_up and m.rsi <= 42 and m.rsi >= RSI_OVERSOLD:
                rsc = 74 + (3 if m.volume_ratio >= VOLUME_STRONG else 0) + (3 if m.rsi_slope > 0 else 0)
                candidates.append((min(rsc,100),d,['برگشت از حمایت','RSI پایین محدوده','کندل برگشتی'],['Range reversal']))
            elif d=='short' and m.edge_reversal_down and m.rsi >= 58 and m.rsi <= RSI_OVERBOUGHT:
                rsc = 74 + (3 if m.volume_ratio >= VOLUME_STRONG else 0) + (3 if m.rsi_slope < 0 else 0)
                candidates.append((min(rsc,100),d,['برگشت از مقاومت','RSI بالای محدوده','کندل برگشتی'],['Range reversal']))
    if not candidates:
