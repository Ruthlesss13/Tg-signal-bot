...
ADX_TREND_MIN = 24          # حداقل ADX برای سیگنال روند مطمئن‌تر
VOLUME_MIN_RATIO = 0.90     # کمی سخت‌تر — حرکت بی‌حجم کمتر
    ...
    while True:
        try:
            for row in open_signals():
                ...
                if hit_sl:
                    tp_note = f"آخرین هدف رسیده‌شده: {highest}" if highest else "هیچ هدفی نخورده بود"
                    text = (
                        f"{rtl}❌ حد ضرر خورد | #{code}\n"
                        f"{rtl}━━━━━━━━━━━━━━━━━━━━\n\n"
                        f"{rtl}📌 جهت: {'خرید (لانگ) 🟢' if direction == 'long' else 'فروش (شورت) 🔴'}\n"
                        f"{rtl}💵 ورود: `{fmt_usd(row['entry'])}`\n"
                        f"{rtl}🛑 حد ضرر: `{fmt_usd(stop_loss)}`\n"
                        f"{rtl}📉 قیمت فعلی: `{fmt_usd(price)}`\n"
                        f"{rtl}📍 {tp_note}\n"
                        f"{rtl}⏱ مدت باز بودن سیگنال: `{duration_text}`\n\n"
                        f"{rtl}سیگنال بسته شد.\n"
                        f"{rtl}📅 `{shamsi_now()}`"
                    )
                    # ریپلای به آخرین پیام این سیگنال (تارگت آخر یا سیگنال اصلی)
                ...
                # ——— تارگت‌ها: TP3→TP2→TP1→سیگنال اصلی ———
                if highest < 1:
                    if (price >= targets[0] if direction == "long" else price <= targets[0]):
                        text = (
                            f"{rtl}1️⃣ هدف اول رسید | #{code}\n"
                            f"{rtl}━━━━━━━━━━━━━━━━━━━━\n\n"
                            f"{rtl}🎯 هدف اول: `{fmt_usd(targets[0])}`\n"
                            f"{rtl}📉 قیمت فعلی: `{fmt_usd(price)}`\n"
                            f"{rtl}🔒 حد ضرر به نقطه ورود منتقل شد: `{fmt_usd(row['entry'])}`\n"
                        ...
                        if CHANNEL_ID and reply_id:
                            ...
                            # آخرین پیام این سیگنال = هدف اول (برای TP2 و هشدار/SL)
                ...
                if highest < 2:
                    if (price >= targets[1] if direction == "long" else price <= targets[1]):
                        text = (
                            f"{rtl}2️⃣ هدف دوم رسید | #{code}\n"
                            f"{rtl}━━━━━━━━━━━━━━━━━━━━\n\n"
                            f"{rtl}🎯 هدف دوم: `{fmt_usd(targets[1])}`\n"
                            f"{rtl}📉 قیمت فعلی: `{fmt_usd(price)}`\n"
                            f"{rtl}🔒 حد ضرر به هدف اول منتقل شد: `{fmt_usd(targets[0])}`\n"
                            f"{rtl}⏱ مدت زمان رسیدن به این هدف: `{duration_text}`\n\n"
                ...
                if (price >= targets[2] if direction == "long" else price <= targets[2]):
                    text = (
                        ...
                        f"{rtl}━━━━━━━━━━━━━━━━━━━━\n\n"
                        f"{rtl}🎯 هدف نهایی: `{fmt_usd(targets[2])}`\n"
                        f"{rtl}📉 قیمت فعلی: `{fmt_usd(price)}`\n"
                ...
                if signal_id not in _trend_warned and CHANNEL_ID and reply_id:
                    try:
                        ...
                        if flipped:
                            if highest == 0:
                                stage = "هنوز هیچ هدفی نخورده"
                            else:
                                stage = f"پس از رسیدن به هدف {highest}"
                            text = (
                                f"{rtl}⚠️ هشدار تغییر روند | #{code}\n"
                                f"{rtl}━━━━━━━━━━━━━━━━━━━━\n\n"
                                f"{rtl}📌 سیگنال باز: {'خرید (لانگ) 🟢' if direction == 'long' else 'فروش (شورت) 🔴'}\n"
                                f"{rtl}📍 وضعیت: {stage}\n"
                                f"{rtl}🔍 دلیل: {' | '.join(reason_bits)}\n"
                                f"{rtl}⏱ مدت باز بودن: `{duration_text}`\n\n"
                                f"{rtl}لطفاً مدیریت ریسک و حد ضرر را بررسی کنید.\n"
                                f"{rtl}📅 `{shamsi_now()}`"
                            )
                            # ریپلای به آخرین پیام این سیگنال (تارگت آخر یا اصلی)
                            msg = await app.bot.send_message(
                                CHANNEL_ID, text, parse_mode="Markdown", reply_to_message_id=reply_id
                            )
                            set_channel_message_id(signal_id, msg.message_id)
                            ...
                            if highest == 0 and ind_4h and (
                                ...
                                inv_text = (
                                    ...
                                    f"{rtl}━━━━━━━━━━━━━━━━━━━━\n\n"
                                    f"{rtl}دلیل: تغییر روند قوی در تایم‌فریم بالاتر\n"
...
async def evaluate_coin(
    ...
    # ——— مسیر ۱: روند صعودی — ADX قوی‌تر + RSI مناسب خرید ———
    if ind_main.adx >= ADX_TREND_MIN and ind_main.trend_up:
        if 35 <= ind_main.rsi <= 65 and ind_main.volume_ratio >= VOLUME_MIN_RATIO:
            direction, setup_type = "long", "trend_up"

    # ——— مسیر ۲: روند نزولی — ADX قوی‌تر + RSI مناسب فروش ———
    if direction is None and ind_main.adx >= ADX_TREND_MIN and ind_main.trend_down:
        if 35 <= ind_main.rsi <= 65 and ind_main.volume_ratio >= VOLUME_MIN_RATIO:
    ...
    if prefer_direction and direction is None:
        ...
        if prefer_direction == "long" and ind_main.trend_up and ind_main.adx >= ADX_TREND_MIN and 35 <= ind_main.rsi <= 65:
            direction, setup_type = "long", "trend_up_reverse"
        elif prefer_direction == "short" and ind_main.trend_down and ind_main.adx >= ADX_TREND_MIN and 35 <= ind_main.rsi <= 65:
    ...
    signal_id = record_signal(
        code, direction, plan.entry, plan.stop_loss, plan.targets,
        plan.leverage, plan.risk_reward, ai_status == "confirmed", ai_text, setup_type,
    )
