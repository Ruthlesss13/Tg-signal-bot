# -*- coding: utf-8 -*-
"""
ربات هوشمند تحلیل فیوچرز ارزهای دیجیتال - نسخه ۳.۳.۴
اصلاحات:
- نرم کردن شرط تقاطع EMA/MACD (lookback چند کندلی)
- تأیید تایم‌فریم بالاتر (۴ ساعته)
- مسیر رنج مستقل‌تر
- OHLCV از فیوچرز (swap) به جای اسپات
- فراخوانی AI غیرمسدودکننده با asyncio.to_thread
- کاهش timeout هوش مصنوعی
"""
...
AI_PROMPT_TEMPLATE = (
    ...
    "CONFIRM or REJECT\n\n"
    "Setup: {direction} {ticker} | RSI:{rsi} | ADX:{adx} | Vol:{volume_ratio}x | Funding:{funding_rate} | OI:{oi_change}\n"
    "Rules: CONFIRM only if ADX>=20 and RSI in 30-70 with clear momentum. Otherwise REJECT.\n"
...
async def evaluate_coin(code: str, account_balance_usdt: float = 1000.0):
    ...
    # مسیر رنج (مستقل — حتی اگر ADX کمی بالاتر باشد ولی reversal واضح باشد)
    ...
    # تأیید تایم‌فریم بالاتر (۴ ساعته) — هم‌جهت بودن
    if direction == "long" and ind_higher and not (ind_higher.trend_up or ind_higher.close > ind_higher.structure_level):
        # اگر ۴ ساعته نزولی واضح باشد، رد کن
        if ind_higher.trend_down and ind_higher.adx >= ADX_RANGE_THRESHOLD:
            return None, "تایم‌فریم ۴ ساعته هم‌جهت نیست (نزولی)", None
    if direction == "short" and ind_higher and not (ind_higher.trend_down or ind_higher.close < ind_higher.structure_level):
        if ind_higher.trend_up and ind_higher.adx >= ADX_RANGE_THRESHOLD:
            return None, "تایم‌فریم ۴ ساعته هم‌جهت نیست (صعودی)", None
