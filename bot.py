...
def init_db():
    with _conn() as c:
        ...
        try:
            ...
            c.execute("UPDATE signals SET outcome_type='tp3', realized_r=COALESCE(realized_r, 0) WHERE status='tp3' AND realized_r IS NULL")
...
def _realized_r_for_outcome(highest_tp: int, final_result: str) -> float:
    # Backward-compatible fallback for legacy rows without target data.
    if highest_tp <= 0: return -1.0
    return round(sum([(TP1_CLOSE_PCT/100)*MIN_RISK_REWARD, (TP2_CLOSE_PCT/100)*MIN_RISK_REWARD*1.5, (TP3_CLOSE_PCT/100)*MIN_RISK_REWARD*2.0][:highest_tp]), 4)
...
async def evaluate_coin(code: str, account_balance_usdt: float=1000.0, skip_cooldown: bool=False, prefer_direction: Optional[str]=None, cooldown_seconds: Optional[float]=None):
    ...
    if btc_regime == "strong_bear" and direction == "long" and setup_type != "range_reversal":
        return None, "رژیم BTC بسیار نزولی است — لانگ آلت‌کوین رد شد", None
    if btc_regime == "strong_bull" and direction == "short" and setup_type != "range_reversal":
...
def report_range(report_type,now=None):
    ...
    if report_type=='weekly':
        start=(now-pd.Timedelta(days=now.weekday())).replace(hour=0,minute=0,second=0,microsecond=0); end=start+pd.Timedelta(days=7)
                ...
                try:
                    ...
                    for _, candle in candles.sort_values("timestamp").iterrows():
                        ...
                        if hit_sl:
                            ...
                            await _finalize_signal(app,row,"loss",close,lesson); closed=True; break
                        ...
                        if new_highest>highest:
                            ...
                            if highest>=3:
                                ...
                                await _finalize_signal(app,row,"win",close,lesson); closed=True; break
        ...
        try:
            ...
            if now.hour==REPORT_HOUR and now.minute>=REPORT_MINUTE:
                if now.weekday()==REPORT_WEEKDAY:
                    start=(now-pd.Timedelta(days=now.weekday())).replace(hour=0,minute=0,second=0,microsecond=0)
