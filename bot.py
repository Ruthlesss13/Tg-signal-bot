...
STRONG_TREND_MIN_SCORE = 48
STRONG_TREND_DI_GAP_MIN = 5.0
STRONG_TREND_FALLBACK_SCORE = 55
...
async def evaluate_coin(code: str, account_balance_usdt: float=1000.0, skip_cooldown: bool=False, prefer_direction: Optional[str]=None, cooldown_seconds: Optional[float]=None):
    ...
    for d in (['long','short'] if not prefer_direction else [prefer_direction]):
        ...
        # ---- 3) vol_momentum: ستاپ جدید برای نوسان جهت‌دار ----
        ...
        if d == 'long':
            vol_mom_ok = (
                ...
                and di_gap_vm >= 3.0
                and 42.0 <= m.rsi <= 70.0
                and m.distance_to_ema_atr <= 2.20
                and m.adx >= 18.0
        ...
        if vol_mom_ok:
            sc_vm = 66 + (6 if m.volume_ratio >= VOLUME_STRONG else 0)
            if structural_vm:
                sc_vm += 4
            if m.adx >= 24:
                sc_vm += 3
            ...
            for row in rows:
                try:
                    ...
                    if not closed:
                        ...
                        # هشدار ساده تغییر جهت قیمت نسبت به ورود (نشانه اولیه تغییر روند)
                        try:
                            if entry > 0 and close > 0:
                                move_pct = (close - entry) / entry * 100
                                if direction == "short":
                                    move_pct = -move_pct
                                # اگر بیش از ۱.۲٪ خلاف جهت سیگنال حرکت کرده باشد
                                if move_pct <= -1.2:
                                    text = (
