...
STRONG_TREND_MIN_SCORE = 55
STRONG_TREND_DI_GAP_MIN = 5.0
STRONG_TREND_FALLBACK_SCORE = 60
STRONG_TREND_SCORE_BONUS = 10
# fallback روند قوی فقط با قدرت واقعی بازار؛ برای range هرگز استفاده نمی‌شود.
STRONG_FALLBACK_ADX_MIN = 30.0
STRONG_FALLBACK_DI_GAP_MIN = 8.0
RSI_LONG_MIN = 50
RSI_LONG_MAX = 68
RSI_SHORT_MIN = 32
RSI_SHORT_MAX = 50
...
# حداکثر اختلاف مجاز قیمت بین لحظه تحلیل و لحظه انتشار سیگنال (۰.۳۵٪)
MAX_PRICE_DRIFT_PCT = 0.0035
...
async def get_fresh_price(code: str, preferred_exchange: Optional[str] = None) -> tuple[Optional[float], str]:
    ...
    for ex, name in exchanges:
        try:
            ticker = await ccxt_call(ex.fetch_ticker, symbol)
...
class MarketCache:
    ...
    async def update_prices(self) -> Dict[str, float]:
        ...
        # ۱. MEXC
        try:
            tickers = await ccxt_call(exchange_mexc.fetch_tickers, symbols_swap)
        ...
        async def fetch_one(code):
            for sym in [f"{code}/USDT:USDT"]:
                try:
                    ticker = await ccxt_call(exchange_mexc.fetch_ticker, sym)
                    if ticker and ticker.get("last") is not None:
                        return code, float(ticker["last"])
                except:
                    pass
                try:
                    ticker = await ccxt_call(exchange_gate.fetch_ticker, sym)
                    if ticker and ticker.get("last") is not None:
                        return code, float(ticker["last"])
                except:
                    pass
...
async def evaluate_coin(code: str, account_balance_usdt: float=1000.0, skip_cooldown: bool=False, prefer_direction: Optional[str]=None, cooldown_seconds: Optional[float]=None):
    ...
    # فیلتر رژیم بازار: در رنج ضعیف ۴H سخت‌گیرانه‌تر باش
    if h is not None and h.regime == "range" and h.adx < 22:
        # در رنج ضعیف ۴H فقط سیگنال‌های خیلی قوی یا range_reversal تمیز اجازه عبور دارند
        if setup_type not in ("range_reversal",) and score < 62:
            return None, f"رژیم رنج ضعیف ۴H (ADX4h={h.adx:.1f}) — امتیاز {score} کافی نیست", None
    ...
    if hist_est.get("pct") is not None and hist_est.get("sample", 0) >= 4 and hist_est["pct"] < 42:
