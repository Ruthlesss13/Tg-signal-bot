...
MIN_SIGNAL_AGE_FOR_TREND_WARN = 5 * 60
...
TREND_WARN_COOLDOWN = 15 * 60          # ۱۵ دقیقه
FUNDING_WARN_COOLDOWN = 30 * 60        # ۳۰ دقیقه
TREND_WARN_THRESHOLD = -1.8            # فقط وقتی بیش از ۱.۸٪ خلاف جهت
TREND_CLEAR_THRESHOLD = -0.8           # hysteresis: بعد از برگشت به این سطح دوباره اجازه هشدار
...
async def evaluate_coin(code: str, account_balance_usdt: float=1000.0, skip_cooldown: bool=False, prefer_direction: Optional[str]=None, cooldown_seconds: Optional[float]=None):
    ...
    # در رنج ضعیف ۴H تقریباً ساکت بمان (فقط range_reversal خیلی تمیز)
    if h is not None and h.regime == "range" and h.adx < 24:
        ...
        if setup_type != "range_reversal":
            return None, f"رژیم رنج ضعیف ۴H (ADX4h={h.adx:.1f}) — فقط برگشت رنج مجاز است", None
        if score < 68:
    ...
    # setup_type از مسیر کاندید آمده (trend_up/down | range_reversal | vol_momentum)
    if not setup_type:
