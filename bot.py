...
async def evaluate_coin(code: str, account_balance_usdt: float=1000.0, skip_cooldown: bool=False, prefer_direction: Optional[str]=None, cooldown_seconds: Optional[float]=None):
    ...
    for d in (['long','short'] if not prefer_direction else [prefer_direction]):
        # ---- 1) range_reversal ----
        if m.regime == 'range':
            # بازار رنج عمداً سخت‌گیرانه‌تر می‌ماند تا افزایش سیگنال‌های روندی
            # باعث ورودهای فیک در کف/سقف‌های ضعیف نشود.
            if (
                d=='long'
                and m.edge_reversal_up
                and RSI_OVERSOLD <= m.rsi <= 42
                and m.rsi_slope > 0
                and m.macd_hist > 0
                ...
                d=='short'
                and m.edge_reversal_down
                and 58 <= m.rsi <= RSI_OVERBOUGHT
                and m.rsi_slope < 0
                and m.macd_hist < 0
                and m.volume_ratio >= VOLUME_MIN_RATIO
            ):
            ...
            # ---- 1b) range_scalp (نوسان‌گیر) — تارگت کوچک مخصوص رنج ----
            # فقط در رژیم رنج؛ نزدیک لبه؛ تأیید مومنتوم خفیف؛ امتیاز متوسط تا جلوی سیگنال روندی معتبر را نگیرد
            if d == 'long' and m.edge_reversal_up and m.rsi <= 45 and m.rsi_slope >= 0 and m.volume_ratio >= 0.90:
                sc_rs = 64 + (4 if m.macd_hist > 0 else 0) + (3 if m.volume_ratio >= 1.10 else 0)
                candidates.append((min(sc_rs, 72), d, ['نوسان‌گیر رنج','نزدیک حمایت','RSI پایین','حجم قابل قبول'], [], 'range_scalp'))
            elif d == 'short' and m.edge_reversal_down and m.rsi >= 55 and m.rsi_slope <= 0 and m.volume_ratio >= 0.90:
                sc_rs = 64 + (4 if m.macd_hist < 0 else 0) + (3 if m.volume_ratio >= 1.10 else 0)
                candidates.append((min(sc_rs, 72), d, ['نوسان‌گیر رنج','نزدیک مقاومت','RSI بالا','حجم قابل قبول'], [], 'range_scalp'))

            # ---- 1c) range_breakout (شکست محدوده) — معامله سریع روی شکست تازه‌ی سقف/کف رنج ----
            # قبلاً این سبک فقط در متن پیام و آمار تعریف شده بود ولی هیچ کاندیدی برایش
            # ساخته نمی‌شد (کد مرده)؛ به همین دلیل هیچ‌وقت سیگنال نمی‌داد.
        ...
        if m.regime != 'range':
            ...
            # روند بسیار قوی یک پاداش محدود می‌گیرد تا نبودن بعضی امتیازهای
            # فرعی مثل 4H/BOS باعث صفر شدن سیگنال‌های معتبر نشود.
            # این پاداش فقط در trend/strong_trend است و هرگز برای range اعمال نمی‌شود.
            di_gap = abs(m.plus_di - m.minus_di)
            rsi_direction_ok = (
                (RSI_LONG_MIN <= m.rsi <= RSI_LONG_MAX) if d=='long'
                else (RSI_SHORT_MIN <= m.rsi <= RSI_SHORT_MAX)
            )
            # fallback مستقل برای روندهای واقعاً قوی
            strong_fallback = (
            ...
            # ---- 3) vol_momentum: ستاپ نوسان جهت‌دار (سخت‌گیرانه‌تر برای کیفیت) ----
            # مناسب بازارهای پرنوسان: حجم قوی + کندل جهت‌دار + مومنتوم RSI/MACD + DI
            # فقط داخل رژیم trend/strong_trend بررسی می‌شود (نه در رنج)، چون این ستاپ
            # ذاتاً برای ادامه‌ی حرکت جهت‌دار طراحی شده و نباید با ستاپ‌های رنج رقابت کند؛
            # با این تفکیک، رژیم بازار مسیر تحلیل را مشخص می‌کند نه اینکه همه ستاپ‌ها
            # همزمان روی هم چک شوند.
            ...
            if d == 'long':
                vol_mom_ok = (
                    m.volume_ratio >= 1.40
                    and m.bullish_candle
                    and m.rsi_slope > 0
                    and m.macd_hist > 0
                    and m.plus_di > m.minus_di
                    and di_gap_vm >= 4.0
                    and 48.0 <= m.rsi <= 65.0
                    and m.distance_to_ema_atr <= 1.90
                    and m.adx >= 22.0
                    and (structural_vm or directional_vm)
                    and not m.macd_cross_down
                )
                vm_reasons = ['حجم قوی نوسانی','کندل صعودی قوی','شیب RSI مثبت','MACD هم‌جهت','DI صعودی']
            else:
                vol_mom_ok = (
                    m.volume_ratio >= 1.40
                    and m.bearish_candle
                    and m.rsi_slope < 0
                    and m.macd_hist < 0
                    and m.minus_di > m.plus_di
                    and di_gap_vm >= 4.0
                    and 35.0 <= m.rsi <= 52.0
                    and m.distance_to_ema_atr <= 1.90
                    and m.adx >= 22.0
                    and (structural_vm or directional_vm)
                    and not m.macd_cross_up
                )
    ...
    if not candidates:
        diag_parts = []
        diag_dirs = ['long','short'] if not prefer_direction else [prefer_direction]
        for diag_d in diag_dirs:
            ...
            effective_diag_score = min(100, dscore + STRONG_TREND_SCORE_BONUS) if strong_fallback else dscore
            if strong_fallback:
                if not structural:
    ...
    # فیلتر رژیم بازار — برای سیگنال مطمئن:
    # در رنج ضعیف ۴H تقریباً ساکت بمان (range_reversal تمیز یا range_scalp)
    if h is not None and h.regime == "range" and h.adx < 24:
        if setup_type == "vol_momentum":
            return None, f"رژیم رنج ضعیف ۴H — ستاپ مومنتوم رد شد", None
        if setup_type not in ("range_reversal", "range_scalp", "range_breakout"):
            return None, f"رژیم رنج ضعیف ۴H (ADX4h={h.adx:.1f}) — فقط برگشت رنج / نوسان‌گیر / شکست محدوده مجاز است", None
        if setup_type == "range_reversal" and score < 68:
            return None, f"رژیم رنج ضعیف ۴H — امتیاز range_reversal کافی نیست ({score}<68)", None
        if setup_type == "range_scalp" and score < RANGE_SCALP_MIN_SCORE:
            return None, f"رژیم رنج ضعیف ۴H — امتیاز نوسان‌گیر کافی نیست ({score}<{RANGE_SCALP_MIN_SCORE})", None
        if setup_type == "range_breakout" and score < RANGE_BREAKOUT_MIN_SCORE:
            return None, f"رژیم رنج ضعیف ۴H — امتیاز شکست محدوده کافی نیست ({score}<{RANGE_BREAKOUT_MIN_SCORE})", None

    if btc_regime == "strong_bear" and direction == "long":
        return None, "رژیم BTC بسیار نزولی است — لانگ آلت‌کوین رد شد", None
    if btc_regime == "strong_bull" and direction == "short":
        return None, "رژیم BTC بسیار صعودی است — شورت آلت‌کوین رد شد", None
    # فیلترهای سخت روندی برای نوسان‌گیر/شکست محدوده اعمال نمی‌شود (منطق خودشان را دارند)
    if setup_type not in ("range_scalp", "range_breakout"):
        ok,reason=check_filters(direction,m,code,funding,oi,entry_price=live_price)
    ...
    # setup_type از مسیر کاندید آمده
    if not setup_type:
    ...
    # توجه: تاریخچه/امتیازدهی گذشته دیگر سیگنال را رد نمی‌کند؛ فقط به‌صورت اطلاعاتی
    # در متن سیگنال (hist_est) نمایش داده می‌شود تا باخت‌های قبلی یک ارز باعث سکوت
    # دائمی سیگنال‌دهی برای آن نشود.
    ...
    if GROQ_API_KEY and AI_GATE_MODE != "off":
        ...
        if AI_GATE_MODE == "veto" and ai_status != "confirmed":
            return None, "AI تأیید نهایی نداد: " + (ai_text or "پاسخ نامعتبر/در دسترس نیست"), None
...
async def _finalize_signal(app: Application, row, result: str, price: float, lesson: str, current_highest: Optional[int] = None):
    ...
    clear_channel_thread(code); _trend_warned.pop(row["id"],None); _funding_warned.pop(row["id"],None)
