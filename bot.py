...
def compute_indicators(df: pd.DataFrame) -> Optional[IndicatorSnapshot]:
    if df is None or len(df) < max(EMA_SLOW,VOLUME_MA_PERIOD,STRUCTURE_LOOKBACK)+21: return None
    # کندل آخر ممکن است هنوز در حال تشکیل باشد؛ برای سیگنال و اندیکاتورها از آخرین کندل بسته‌شده استفاده کن.
...
def setup_score(direction: str, m: IndicatorSnapshot, h: IndicatorSnapshot, funding: Optional[float]) -> tuple[int,list[str],list[str]]:
    ...
    checks=[
        ...
        (m.adx_slope>=ADX_SLOPE_MIN,4,'ADX رو به افزایش'),
        ...
        (m.volume_ratio>=VOLUME_TREND_MIN,5,'حجم تأییدکننده روند'),
...
def check_filters(direction: str, indicators: IndicatorSnapshot, code: str, funding_rate: Optional[float]=None, oi: Optional[float]=None) -> tuple[bool,str]:
    ...
    if indicators.volume_ratio < VOLUME_TREND_MIN:
        return False, f"حجم تأییدکننده نیست: {indicators.volume_ratio:.2f}x < {VOLUME_TREND_MIN:.2f}x"
    if indicators.adx < ADX_TREND_MIN or indicators.adx_slope < ADX_SLOPE_MIN:
        return False, f"ADX/مومنتوم روند کافی نیست: ADX={indicators.adx:.1f} slope={indicators.adx_slope:+.2f}"
...
# ===========================
def get_signal_strength_and_confidence(ind: IndicatorSnapshot, funding: Optional[float], oi: Optional[float], rule_score: Optional[int]=None) -> tuple[str,str,int]:
    d='long' if ind.trend_up else 'short'
    # اگر امتیاز واقعی ستاپ در زمان صدور موجود است، همان را نمایش بده؛
    # قبلاً با پاس دادن ind به‌عنوان تایم‌فریم 4H امتیاز ساختاری بیش‌برآورد می‌شد.
    score = int(rule_score) if rule_score is not None else setup_score(d,ind,ind,funding)[0]
...
def format_signal_message(code: str, direction: str, plan: RiskPlan, setup_type: str, ai_status: str, ai_text: str, ind: IndicatorSnapshot, funding: Optional[float], oi: Optional[float], rate: float, hist_est: Optional[dict] = None, rule_score: Optional[int] = None) -> str:
    ...
    strength, confidence_text, score = get_signal_strength_and_confidence(ind, funding, oi, rule_score=rule_score)
...
async def evaluate_coin(code: str, account_balance_usdt: float=1000.0, skip_cooldown: bool=False, prefer_direction: Optional[str]=None, cooldown_seconds: Optional[float]=None):
    ...
    for d in (['long','short'] if not prefer_direction else [prefer_direction]):
        if m.regime == 'range':
            ...
            higher_tf_ok=(h.trend_up if d=='long' else h.trend_down)
            adx_ok=m.adx>=ADX_TREND_MIN and m.adx_slope>=ADX_SLOPE_MIN
            volume_ok=m.volume_ratio>=VOLUME_TREND_MIN
            # بعد از خطای APT، ورود بدون تأیید ساختار دیگر مجاز نیست.
            # هم‌جهتی 4H، ADX رو به رشد و حجم نیز برای ستاپ روند اجباری‌اند.
            hard_ok=(
                structural if REQUIRE_STRUCTURE_CONFIRMATION else True
            ) and (
                higher_tf_ok if REQUIRE_4H_ALIGNMENT else True
            ) and adx_ok and volume_ok
            if sc>=MIN_SIGNAL_SCORE and hard_ok:
                candidates.append((sc,d,rs,wr))
    if not candidates:
        return None,f'ستاپ رد شد | score<{MIN_SIGNAL_SCORE} یا ساختار/4H/ADX/حجم کافی نیست | ADX={m.adx:.1f} slope={m.adx_slope:+.2f} RSI={m.rsi:.1f} vol={m.volume_ratio:.2f}x +DI/-DI={m.plus_di:.1f}/{m.minus_di:.1f} BOS={m.bos_up}/{m.bos_down}',None
    candidates.sort(reverse=True); score,direction,reasons,warnings=candidates[0]
    # فیلتر نهایی قبل از ساخت سیگنال: حتی اگر امتیاز عبور کند، ستاپ بدون ساختار/4H مجاز نیست.
    structural_final=(m.bos_up or m.retest_long) if direction=='long' else (m.bos_down or m.retest_short)
    higher_tf_final=(h.trend_up if direction=='long' else h.trend_down)
    if REQUIRE_STRUCTURE_CONFIRMATION and not structural_final:
        return None, "ورود رد شد: BOS/Retest تأیید نشده است", None
    if REQUIRE_4H_ALIGNMENT and not higher_tf_final:
        return None, "ورود رد شد: روند 4H هم‌جهت نیست", None
    ...
    features = {
        ...
        "adx_slope": round(m.adx_slope, 3), "h4_aligned": bool(higher_tf_final),
        "structural_confirmed": bool(structural_final),
    }
...
async def _publish_signal(app: Application, signal_id: int, payload: tuple):
    ...
    if not code:
        return
    rule_score = payload[10] if len(payload) > 10 else None
    text = format_signal_message(code, direction, plan, setup_type, ai_status, ai_text, ind, funding, oi, rate, hist_est=hist_est, rule_score=rule_score)
    sent_channel = None
