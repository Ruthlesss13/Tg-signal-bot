...
async def evaluate_coin(code: str, account_balance_usdt: float=1000.0, skip_cooldown: bool=False, prefer_direction: Optional[str]=None, cooldown_seconds: Optional[float]=None):
    ...
    if not candidates:
        # v4.1.2: breakdown کامل امتیاز برای دیباگ؛ هیچ شرط ورود/فیلتر ضعیف نشده است.
        # این لاگ فقط نشان می‌دهد هر جزء از setup_score چرا امتیاز گرفته/نگرفته است.
        for _d in (['long', 'short'] if not prefer_direction else [prefer_direction]):
            _sc, _rs, _wr = setup_score(_d, m, h, funding)
            _direction_fa = 'LONG' if _d == 'long' else 'SHORT'
            _structural = (m.bos_up or m.retest_long) if _d == 'long' else (m.bos_down or m.retest_short)
            _strong_trend = (
                (m.trend_up if _d == 'long' else m.trend_down)
                and (m.plus_di > m.minus_di + MIN_DI_GAP if _d == 'long' else m.minus_di > m.plus_di + MIN_DI_GAP)
                and m.adx >= ADX_STRONG
            )
            _missing = []
            _expected = {
                'روند 1H هم‌جهت': 18, 'روند 4H هم‌جهت': 14, 'DI هم‌جهت': 10,
                'قدرت روند': 8, 'ساختار/پولبک تأییدشده': 18,
                'مومنتوم RSI در بازه': 10, 'MACD هم‌جهت بدون تقاطع مخالف': 8,
                'حجم کافی': 5, 'حجم قوی': 4, 'ورود نزدیک به EMA': 5,
                'فاندینگ به نفع جهت': 2,
            }
            for _label, _pts in _expected.items():
                if _label not in _rs:
                    _missing.append(f'{_label}(-{_pts})')
            _logger_msg = (
                f'🔬 {_direction_fa} breakdown | score={_sc}/100 | ' +
                f'passed={",".join(_rs) if _rs else "هیچ"} | ' +
                f'missing={",".join(_missing) if _missing else "هیچ"} | ' +
                f'structural={_structural} strong_trend={_strong_trend}'
            )
            logger.info(f'📊 {code}: {_logger_msg}')
