...
def confirm_signal_with_ai(chart_png: Optional[bytes], context: dict) -> tuple[str, str]:
    """بدون response_format — مدل‌های gpt-oss روی json_mode خطای validate می‌دهند."""
    if not GROQ_API_KEY:
    ...
    prompt = (
        f"Review this crypto futures setup for a {expected} signal.\n"
        "Answer in exactly this format on one line (no markdown):\n"
        f"DECISION={expected}_or_NO_TRADE CONF=0to100 REASON=short_Persian\n"
        "Reject if weak ADX, stretched RSI, no structure, or poor RR.\n"
        f"DATA: {json.dumps(compact, ensure_ascii=False, default=str)}"
    )
    ...
    payload = {
        ...
        "messages": [
            {
                "role": "system",
                "content": "You are a strict crypto futures filter. Reply with one plain line only.",
            },
    ...
    try:
        ...
        # تلاش برای JSON اگر مدل برگرداند
        obj = None
        try:
            cleaned = raw_s
        ...
        if obj and isinstance(obj, dict):
            ...
            text = (reason or decision) + f" | ارزیابی AI: {conf}%"
            if decision == expected:
                return "confirmed", text[:800]
            return "rejected", text[:800]

        # پارس خط متنی: DECISION=... CONF=... REASON=...
        up = raw_s.upper()
        conf = 50
        m_conf = re.search(r"CONF\s*[=:]\s*(\d{1,3})", up)
        if m_conf:
            conf = max(0, min(100, int(m_conf.group(1))))
        reason = raw_s
        m_reason = re.search(r"REASON\s*[=:]\s*(.+)$", raw_s, flags=re.I | re.S)
        if m_reason:
            reason = m_reason.group(1).strip()[:400]

        if "NO_TRADE" in up or "REJECT" in up or "NO TRADE" in up:
            return "rejected", f"{reason} | ارزیابی AI: {conf}%"
        if expected in up:
            return "confirmed", f"{reason} | ارزیابی AI: {conf}%"
        # اگر واضح نبود — رد محافظه‌کارانه نه unavailable
        return "rejected", f"{reason[:300]} | ارزیابی AI: {conf}%"
