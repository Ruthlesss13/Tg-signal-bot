...
async def _render_stats_background(context, query, key, version):
    try:
        ...
        text = (
            ...
            f"💰 فاکتور سود (Profit Factor): `{pf_txt}`\n"
...
async def evaluate_coin(code: str, account_balance_usdt: float=1000.0, skip_cooldown: bool=False, prefer_direction: Optional[str]=None, cooldown_seconds: Optional[float]=None):
    ...
    if hist_est.get("pct") is not None and hist_est.get("sample", 0) >= 4 and hist_est["pct"] < 48:
...
def stats_summary() -> dict:
    with _conn() as c:
        ...
        # میانگین نسبت سود به زیان هدف‌گذاری‌شده روی معاملات بسته‌شده
        rr_row = c.execute(
            "SELECT AVG(risk_reward) AS avg_rr FROM signals WHERE status IN ('tp3','sl')"
        ).fetchone()
        # سود ناخالص واقعی‌شده بر حسب R (هر برد = risk_reward همان سیگنال) و تعداد باخت‌ها (هر باخت = ۱R)
        pf_row = c.execute(
            "SELECT "
            "SUM(CASE WHEN status='tp3' THEN risk_reward ELSE 0 END) AS gross_win, "
            "SUM(CASE WHEN status='sl' THEN 1 ELSE 0 END) AS gross_loss_n "
            "FROM signals WHERE status IN ('tp3','sl')"
        ).fetchone()
        # نرخ موفقیت به تفکیک بازه امتیاز ورود (از جدول experiences)
        score_rows = c.execute(
            "SELECT entry_score, result FROM experiences WHERE entry_score IS NOT NULL"
        ).fetchall()
    ...
    avg_rr = rr_row["avg_rr"] if rr_row and rr_row["avg_rr"] is not None else None
    gross_win = (pf_row["gross_win"] or 0.0) if pf_row else 0.0
    gross_loss_n = (pf_row["gross_loss_n"] or 0) if pf_row else 0
    if gross_loss_n:
        profit_factor = gross_win / gross_loss_n
    else:
        profit_factor = float("inf") if gross_win else None
    # بازه‌بندی امتیاز: هر بازه ۱۰ نمره‌ای
    ...
    for row in score_rows:
        ...
        lo = int(sc // 10) * 10
        label = f"{lo}-{lo+9}"
        b = buckets.setdefault(label, {"win": 0, "loss": 0})
        b["win" if row["result"] == "win" else "loss"] += 1
    score_lines = []
    for label in sorted(buckets.keys(), key=lambda x: int(x.split("-")[0])):
        b = buckets[label]
        n = b["win"] + b["loss"]
        wr = f"{(b['win']/n*100):.0f}%" if n else "—"
        score_lines.append(f"• {label}: کل {n} | برد {b['win']} | باخت {b['loss']} | نرخ {wr}")
    return {
...
async def _finalize_signal(app: Application, row, result: str, price: float, lesson: str):
    ...
    # پاک کردن پرچم‌های هشدار برای این سیگنال
