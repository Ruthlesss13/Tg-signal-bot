...
def check_filters(direction: str, indicators: IndicatorSnapshot, code: str, funding_rate: Optional[float]=None, oi: Optional[float]=None, entry_price: Optional[float]=None) -> tuple[bool,str]:
    ...
    if indicators.atr <= 0 or live_entry_distance > MAX_ENTRY_DISTANCE_ATR:
        return False, f"فاصله ورود از EMA زیاد است: {live_entry_distance:.2f} ATR"
...
async def evaluate_coin(code: str, account_balance_usdt: float=1000.0, skip_cooldown: bool=False, prefer_direction: Optional[str]=None, cooldown_seconds: Optional[float]=None):
    ...
    if not candidates:
        ...
        for diag_d in diag_dirs:
            ...
            missing_text = ', '.join(missing[:4]) if missing else 'همه معیارهای امتیازدهی عبور کرده‌اند'
...
def main():
    ...
    logger.info(f"🎯 Signal filters: score>={MIN_SIGNAL_SCORE} | ADX>={ADX_TREND_MIN} | DI gap>={MIN_DI_GAP} | EMA distance<={MAX_ENTRY_DISTANCE_ATR} ATR | version={VERSION}")
